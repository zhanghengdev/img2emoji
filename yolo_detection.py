...
def load_detector(cfg="cfg/yolov3.cfg", weights="yolov3.weights", data="cfg/coco.data", thresh=.5, hier_thresh=.5, nms=.45):
    net = load_net(cfg, weights, 0)
    meta = load_meta(data)
    return net, meta

def detect_img(filename, cfg="cfg/yolov3.cfg", weights="yolov3.weights", data="cfg/coco.data", thresh=.5, hier_thresh=.5, nms=.45):
    net = load_net(cfg, weights, 0)
    meta = load_meta(data)
    start=time.time()
...
def detect_imgs(filenames, cfg="cfg/yolov3.cfg", weights="yolov3.weights", data="cfg/coco.data", thresh=.5, hier_thresh=.5, nms=.45):
    net = load_net(cfg, weights, 0)
    meta = load_meta(data)
    rs = []
    for filename in filenames:
        r = detec_img_with_preloaded_detector(filename, net, meta, thresh, hier_thresh, nms)
        rs.append(r)
    return rs
